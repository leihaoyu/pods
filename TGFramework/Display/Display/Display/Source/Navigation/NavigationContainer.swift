...
public final class NavigationContainer: ASDisplayNode, UIGestureRecognizerDelegate {
    ...
    public func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldBeRequiredToFailBy otherGestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        if let _ = otherGestureRecognizer as? UIPanGestureRecognizer {
            if String(describing: type(of: otherGestureRecognizer)).contains("TIOPanGestureRecognizer") {
                return false
            }
            print("--------\(otherGestureRecognizer.view)")
            if let scroo = otherGestureRecognizer.view as? UIScrollView {
                if scroo.contentOffset.x > 0 {
                    return false
                }
            }
            if let table = otherGestureRecognizer.view as? UITableView {
                if table.isEditing {
                    return false
                }
            }
            print("------true--\(otherGestureRecognizer.view)")
            return true
        }
        return false
    }
