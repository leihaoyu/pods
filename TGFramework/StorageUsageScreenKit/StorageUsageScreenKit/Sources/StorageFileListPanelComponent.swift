...
import TelegramCore
import MultilineTextComponentKit
import EmojiStatusComponentKit
import PostboxKit
import TelegramStringFormatting
import CheckNodeKit
import AvatarNodeKit
import PhotoResources
import SemanticStatusNodeKit
    ...
    final class View: ContextControllerSourceView {
        ...
        override init(frame: CGRect) {
            ...
            self.extractedContainerView.willUpdateIsExtractedToContextPreview = { [weak self] value, transition in
                ...
                if value {
                    mappedTransition = TGTransition(transition)
                } else {
                    mappedTransition = TGTransition(animation: .curve(duration: 0.2, curve: .easeInOut))
                }
        ...
        func update(component: FileListItemComponent, availableSize: CGSize, state: EmptyComponentState, environment: Environment<Empty>, transition: TGTransition) -> CGSize {
...
final class StorageFileListPanelComponent: Component {    
    ...
    class View: UIView, UIScrollViewDelegate {
        ...
        func update(component: StorageFileListPanelComponent, availableSize: CGSize, state: EmptyComponentState, environment: Environment<StorageUsagePanelEnvironment>, transition: TGTransition) -> CGSize {
            self.component = component
    ...
    func update(view: View, availableSize: CGSize, state: EmptyComponentState, environment: Environment<StorageUsagePanelEnvironment>, transition: TGTransition) -> CGSize {
        return view.update(component: self, availableSize: availableSize, state: state, environment: environment, transition: transition)
    }
}
